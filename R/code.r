...
Lval <- function(x, method = mean) {
  ...
  m <- numeric(d)
  for (i in 1:d) {
    rho <- acf(x[,i], lag.max = lagmax, type = "correlation", plot = FALSE)$acf[-1]
...
E1hat <- function(Z, h) {
  ...
  Zmat <- t(matrix(Z, N, N))
  Zmat <- Z - Zmat
  Zmat <- dnorm(Zmat, sd = sqrt(2) * h)
  diag(Zmat) <- 0
  E1 <- 0
  for(i in 1:(N-1)){
    for(j in (i+1):N){
      t1 <- dnorm(Z[i] - Z[j], sd = sqrt(2) * h)
...
E2hat <- function(Z, h){
  ...
  for(i in 1:(N-1)){
    for(j in (i+1):N){
      ...
      t2  <- dnorm(Z[i] - Z[vec], sd = sqrt(2) * h)
      t3  <- dnorm(Z[j] - Z[vec], sd = sqrt(2) * h)
...
E3hat <- function(Z, h){
  ...
  for(i in 1:(N-1)){
    for(j in (i+1):N){
      t1 <- dnorm(Z[i] - Z[j], sd = sqrt(2) * h) ^ 2
      E3 <- E3 + t1
    }
  }
  Number <- N * (N - 1) / 2
...
Ji <- function(x, y, h) {
  ...
  X <- t(matrix(x, n, n))
  X <- x - X
  X <- dnorm(X, sd = sqrt(2) * h)
  diag(X) <- 0
  t1 <- sum(X) / (n * (n - 1))
  Y <- t(matrix(y, m, m))
  Y <- y - Y
  Y <- dnorm(Y, sd = sqrt(2) * h)
  diag(Y) <- 0
  t2 <- sum(Y) / (m * (m - 1))
  XY <- x[1] - y
  for (j in 2:n) {
